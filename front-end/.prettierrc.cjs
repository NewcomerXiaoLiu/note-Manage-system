...
module.exports = {
  ...
  singleAttributePerLine: true, // 每个 HTML/XML 属性应该单独占一行
  vueIndentScriptAndStyle: true, // 控制在 Vue 单文件组件中 <script> 和 <style> 标签内的代码缩进方式
  quoteProps: 'consistent', // 在对象字面量中决定是否将属性名用引号括起来 可选值 "<as-needed|consistent|preserve>"
  jsxSingleQuote: true, // 在JSX中使用单引号而不是双引号 (true：单引号，false：双引号)
